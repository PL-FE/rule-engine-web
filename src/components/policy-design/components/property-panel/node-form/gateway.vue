<template>
  <div>
    ...
    <div v-for="(c, idx) in children" :key="c.id" class="flex mt-3 items-center">
      ...
      <el-select class="!w-48 mr-3" v-model="data[idx].varValue" placeholder="请选择变量">
        <el-option
      ...
      <el-select class="!w-48 mr-3" v-model="data[idx].judge" placeholder="请选择判断符">
        <el-option
      ...
      <el-input class="!w-48 mr-3" v-model="data[idx].value" placeholder="请输入值" />
      <el-input class="!w-48 mr-3" v-model="data[idx].remark" placeholder="备注" />
    </div>
  </div>
</template>
